...
datasets.BASE_DATA_FOLDER = '/home/fuhao/workspace-4gpu/data'
SAVED_MODELS_FOLDER = '/home/fuhao/workspace-4gpu/projects/quantization/Bayesian_quant/models/'
cifar10Manager = model_manager.ModelManager('./models/Bayesian_distill_cifar10/model_manager_Bayesian_distill_cifar10.tst',
                                            'model_manager', create_new_model_manager=True)
cifar10modelsFolder = os.path.join(SAVED_MODELS_FOLDER, 'Bayesian_distill_cifar10')
try:
...
print("# --------------------------- Teacher model [cifar10_teacher_resnet101]---------------------- #")
model_name = 'cifar10_teacher_resnet101'
teacherModelPath = os.path.join(cifar10modelsFolder, model_name)
teacherModel = ResNet101()
...
print("# -------------------------2 Student [cifar10_student_ResNet18 standard distillation] ---------------------- #")
if MY_TRAIN_DISTILL_RESNET18_BETTER:
    print("#### -------------------------2 ResNet18 + distilled [1] [Better Teacher]-----------------------")
    # smaller and distilled
    # smallerOptions = {'widen_factor':22, 'depth':16, 'dropout_rate':0.3, 'num_classes':10}
    
    distilled_model_name = 'my_cifar10_distill_better_resnet18'
    distilled_model_path = os.path.join(cifar10modelsFolder, distilled_model_name)
    # distilledModel_res18 = Wide_ResNet(**smallerOptions)
    distilledModel_res18 = ResNet18()
    if USE_CUDA: distilledModel_res18 = distilledModel_res18.cuda()
    if NUM_GPUS > 1: distilledModel_res18 = torch.nn.parallel.DataParallel(distilledModel_res18)
    ...
    # if MY_TRAIN_DISTILL_1:
    cifar10Manager.train_model(distilledModel_res18, model_name=distilled_model_name,
                               train_function=convForwModel.train_model,
                               arguments_train_function={'epochs_to_train': 100,
                                                         'print_every':50,
                                                         'initial_learning_rate':0.1,
                                                         'learning_rate_style':'cifar100',
                                                         'weight_decayL2':0.0005,
                                                         'teacher_model': teacherModel_res101,
                                                         'use_distillation_loss': True},
                               train_loader=train_loader, test_loader=test_loader)
    #distilledModel_res18.load_state_dict(cifar10Manager.load_model_state_dict(distilled_model_name))
    del distilledModel_res18
